...
function bindNetwork() {
  ...
  const render = (notify) => {
    ...
    if (notify) {
      toast(online ? '已恢复网络连接' : '离线模式 · 行程与预约码仍可查看');
    }
  };
...
function renderHeader() {
  ...
  $('#tripTitle').textContent = trip.title || '日本旅行';
  $('#tripSub').textContent   = trip.subtitle || '';
...
function renderDay() {
  ...
  $('#dayTitle').textContent = `${day.label} · ${day.title || ''}`;
  $('#daySub').textContent   = day.subtitle || day.city || '';
  $('#dayCount').textContent = `${(day.items || []).length} 项`;
...
function renderMap(day) {
  ...
  /* 视野 */
  if (pts.length === 0) {
    STATE.map.setView(day.center || [35.6812, 139.7671], day.zoom || 12);
  } else if (pts.length === 1) {
    STATE.map.setView(pts[0], 15);
  } else {
    STATE.map.fitBounds(L.latLngBounds(pts), { padding: [36, 36], maxZoom: 16 });
  }
